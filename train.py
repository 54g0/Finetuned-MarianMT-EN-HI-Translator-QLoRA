...
class Datapreprocessor:
    def __init__(self, config: Config):
        ...
        if config.data_limit:
            self.dataset["train"] = self.dataset["train"].select(range(config.data_limit))
            self.dataset["test"] = self.dataset["test"].select(range(int(config.data_limit*0.1)))
    ...
    def get_dataset(self):
        return self.dataset.map(self.preprocess, batched=True,
...
class ModelManager:
    ...
    def setup_model_and_tokenizer(self):
        ...
        lora_config = LoraConfig(r=self.config.lora_r,
...
if __name__ == "__main__":
    ...
    print("Training complete. Metrics:", eval_results)